/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
Command: npx gltfjsx@6.5.2 public/three/daniel-whole.gltf -t -m --shadows -d
*/
import { useGLTF } from "@react-three/drei";
import { useLoader } from "@react-three/fiber";
import { Mesh, MeshStandardMaterial } from "three";
import { GLTF, GLTFLoader } from "three-stdlib";

import { GLTFAction } from "@/types/global";

type GLTFResult = {
  animations: GLTFAction[];
  materials: {
    ["Outer.009"]: MeshStandardMaterial;
  };
  nodes: {
    Daniel_Whole: Mesh;
  };
} & GLTF;

const PATH = "/three/daniel-whole.min.gltf";

export function DanielWhole(props: JSX.IntrinsicElements["group"]) {
  const { materials, nodes } = useLoader(GLTFLoader, PATH) as GLTFResult;
  return (
    <group {...props} dispose={null}>
      <mesh
        castShadow
        geometry={nodes.Daniel_Whole.geometry}
        material={materials["Outer.009"]}
        material-wireframe={true}
        receiveShadow
        rotation={[Math.PI / 2, 0, 0]}
        scale={0.1}
        userData={{ name: "Daniel_Whole" }}
      />
    </group>
  );
}

useGLTF.preload(PATH);
